...
def table_maker(ticker):
	global con
	#connect to database, but create database if not found
	try:
		con = mysql.connector.connect(user='root', password='waWWii21156!', host='127.0.0.1', database=ticker)
#		con = mysql.connector.connect(user='maxuser', password='waWWii21156!', host='35.231.41.51', database=ticker)
		print("Database " +ticker+ " connected.")
	except Exception as e:
		print("Timeout:",getattr(e, 'message', repr(e)))
		con = mysql.connector.connect(user='root', password='waWWii21156!', host='127.0.0.1', database=ticker)
#		con = mysql.connector.connect(user='maxuser', password='waWWii21156!', host='35.231.41.51', database="flagler")
		mycursor = con.cursor()
...
#				con = mysql.connector.connect(user='maxuser', password='waWWii21156!', host='35.231.41.51', database=ticker)	
	...
	try:
		cursor = con.cursor()
		cursor.execute('''
		CREATE TABLE IF NOT EXISTS `'''+ticker+'''_hist_polygon` (
		`LAST_PRICE` decimal(10,2) NOT NULL default 0,
		`LAST_SIZE` decimal(10,0) NOT NULL default 0,
		`LAST_TIME` bigint(20) NOT NULL default 0,
		`sequence` int(11) NOT NULL default 0,
		`id` bigint(40) NOT NULL default 0,
		`exchange` int(4) NOT NULL default 0,
		`condition` char(20)  default NULL,
...
#				last_condition = val
		...
		try:
			cursor = con.cursor()
			# Create a new record
			sql = "INSERT INTO `"+ticker.lower()+"`.`"+ticker.lower()+"_hist_polygon` (`LAST_PRICE`, `LAST_SIZE`, `LAST_TIME`, `sequence`, `id`, `exchange`, `condition`) VALUES (%s, %s, %s, %s, %s,%s, %s)"
			data = (
			last_price,
			last_size,
			last_time,	
...
if tickers is not None:
	for ticker in tickers:
		table_maker(ticker)
		print("making table:",tickers)
	tickers = ["T."+item.upper() for item in tickers]
	tickers = ','.join(tickers)
else:
	table_maker(ticker)
	print("making a table:",ticker)
	#tickers = "T."+ticker.upper()
