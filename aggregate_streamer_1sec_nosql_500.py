#on mini1, in mysql database:
# mysql -uroot -pwaWWii21156! ticker_prices;
#get the top 100 today by volume:
#select symbol,FROM_UNIXTIME(unixtime/1000),volume,close from closing_aggs where FROM_UNIXTIME(unixtime/1000,'%Y-%m-%d') = CURDATE()  order by volume desc limit 100;
#save to file
#import to excel to paste to one entry symbol per line (dump100.txt)
#run: awk '{print $1}' dump100.txt | paste -s -d, -
#XELA,F,KACLR,MOTO,MORT,MRAD,FEMB,SMSA,PACI,NICOF,OTGAS,EBON,EBSB,EDU,ET,AMD,VCNX,BBIG,VAMO,UVXY,T,DKNG,AAPL,CLVS,TQQQ,EEMD,EEM,AMC,VXX,NIO,SPY,ASAQ,NOM,NOK,MSFT,PLTR,HEWU,FCEL,DNAA,BKKT,QQQ,NLY,LCID,NVDA,JNPR,SH,GLTA.U,GLTA,ITUB,SOXS,WISH,LYG,RIO,SNAP,WPCA.U,WPCA,PBR,HYG,ALGCF,YLLWF,ARGU,BMAC,PXSNF,DSAQ,SNGGF,MCAGU,BRNHF,TSCLF,SWRHF,IPAX,STMAF,VEON,BMAQR,CCL,CCM,GFI,HYGV,TLGY,EWZ,SOFI,HFBL,HEXO,DNB,REET,RIOT,RIVE,FXI,INFY,PTON,CLVT,PDD,PDEV,KGC,GATO,BAC,BACA.U,PSQ,BCS,UBER,SNDL
...
file_exists = os.path.exists('symbols.csv')
if file_exists == True:
	...
	print("symbols.csv doesnt exist!")
	exit()
