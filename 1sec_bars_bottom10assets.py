...
path1 = os.path.dirname(os.path.abspath(inspect.getfile(inspect.currentframe())))	#this is current script directory and more reliable than hard paths - dont hard code path! - ben
file1 = "bottom100_" +ydate+ ".csv"
topassets = pd.read_csv(file1)

print(topassets.head(10))
toptickers = topassets['ticker']

print(toptickers.head(10))
spy = toptickers.head(10)
...
spy = newspy
#spy = "GLSI,EXPR,EYES,CHS,GME,ANY,MOXC,JAGX,KOSS,BSQR"
...
#							out = str(msg["vol"]) + ", " + str(msg["av_vol"]) + ", " + str(msg["open_price"]) + ", "
							out += str(msg["open"]) + ", " + str(msg["high"]) + ", " + str(msg["low"]) + ", "
...
# 							f[msg["symbol"]].write(out)
						...
						try:
							...
							print ("sql:", (sql, data))
							out = str(msg["price"]) + ", " +str(msg["size"])+ "," +str(int(msg["timestamp"]/1000))+"\n"
...
def main():  
	...
	os.environ['polygon_api_key'] = "jg0i3J_ZFD1_v4wBRQFEzp9NHDlYwHQff5_8_u" # Dan
#	os.environ['polygon_api_key'] = "J2M4jq6ltDM7c9VlboKAFIUklyxvpIdX" # Jerzy
#	os.environ['polygon_api_key'] = "1EpFWZtwtEPwn_XvbaBPDHb7uveSB8Yk" # Ben
#	os.environ['polygon_api_key'] = "L_xioaVuXUrSRzrzCut5hiAFO2WtfIOP" # Zhenya
