#this gets order from the short account
...
os.environ["APCA_API_KEY_ID"] = "AK4QNV0VH1JH09SBD9JB"
os.environ["APCA_API_SECRET_KEY"] = "tiRlTQGI5U7lbzrQPn8x5jyDcoKLPEH3Mc/W/QNP"
...
def writeData(order):
	...
	try:
		# Create a new record
		cursor = con.cursor()
		sql = "INSERT INTO `orders_short_"+ticker+"` (`asset_id`, `canceled_at`, `client_order_id`, `created_at`, `expired_at`, `extended_hours`, `failed_at`, `filled_at`, `filled_avg_price`, `filled_qty`, `id`, `legs`, `limit_price`, `order_type`, `qty`, `replaced_at`, `replaced_by`, `replaces`, `side`, `status`, `stop_price`, `submitted_at`, `symbol`, `time_in_force`, `type`, `updated_at`) VALUES (%s, %s, %s, %s, %s,%s, %s, %s, %s, %s,%s, %s, %s, %s, %s,%s, %s, %s, %s, %s,%s, %s, %s, %s, %s,%s)"
...
try:
	cursor = con.cursor()
	cursor.execute('''
		CREATE TABLE IF NOT EXISTS `orders_short_'''+ticker+'''` (
